...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	nowNumber := new(big.Int).SetUint64(number)
	if(nowNumber.Cmp(replayNumber) > 0) {
		...
		body := GetBodyOld(db, hash, number)
		if body == nil {
			return nil
		}
		transactions := types.NewTransactionsOld(body.Transactions)
		// Reassemble the block and return
		return types.NewBlockWithHeader(header).WithBody(transactions, body.Uncles)
	}
}
...
func GetTransactionOld(db DatabaseReader, hash common.Hash,blockHash common.Hash,blockNumber uint64,txIndex uint64) (*types.Transaction) {
	...
	if blockHash != (common.Hash{}) {
		body := GetBodyOld(db, blockHash, blockNumber)
		// if body == nil || len(body.Transactions) <= int(txIndex) {
		if body == nil {
			log.Error("Transaction referenced missing", "number", blockNumber, "hash", blockHash, "index", txIndex)
			return nil
		}
		txNew := types.NewTransactionOld(body.Transactions[txIndex])
		return txNew
	...
	// txNew := types.NewTransactionOld(tx)
...
func GetTransaction(db DatabaseReader, hash common.Hash) (*types.Transaction, common.Hash, uint64, uint64) {
	...
	if blockHash != (common.Hash{}) {
		...
		if(nowNumber.Cmp(replayNumber) > 0) {
			...
			txNew := types.NewTransactionOld(body.Transactions[txIndex])
			return txNew, blockHash, blockNumber, txIndex
		}
